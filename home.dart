...
class _HomeState extends State<Home> {
  ...
  Future<List> getData() async {
    final response =
        await http.get("http://192.168.43.63/apiflutter/api/Penjualankucing");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: new AppBar(
        title: new Text("Penjualan Sepatu Futzal"),
      ),
      body: createListView(),
      //tombol add
  ...
  Future<http.Response> deletePenjualan(id) async {
    final http.Response response = await http
        .delete('http://192.168.43.65/apiflutter/api/PenjualanSepatuFutzal/delete/$id');
    return response;
  }
